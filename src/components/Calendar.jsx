...
import { getWeatherForecast, getWeatherAlerts } from '../utils/weatherAPI';  // Gets weather data
...
const Calendar = ({ calendarData, updateCalendarData, weather }) => {
  ...
  useEffect(() => {
    // Get weather predictions for the next 14 days
    const forecast = getWeatherForecast(14, weather?.location || 'Karnataka');
    setWeatherForecast(forecast);  // Save the weather data
    
    // Check if there are any weather warnings (storms, extreme temperatures, etc.)
    const alerts = getWeatherAlerts(forecast);
    setWeatherAlerts(alerts);  // Save the weather alerts
  }, [weather?.location]);  // This runs whenever the location changes
