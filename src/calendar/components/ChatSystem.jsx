...
const ChatSystem = () => {
  ...
  // This creates user messages and simulates AI bot responses
  const handleSendMessage = async () => {
    // Don't send empty messages or if no bot is selected
    if (!inputMessage.trim() || !selectedBot) return;
  
    // Create a new message object from the user's input
    const userMessage = {
      id: Date.now(),                    // Unique identifier using current timestamp
      text: inputMessage,                // The actual message text the user typed
    ...
    setInputMessage('');                           // Clear the typing area
    setIsTyping(true);                            // Show typing indicator
  
    // Simulate bot response with a realistic delay
    // This makes the conversation feel more natural, like the AI is thinking
    setTimeout(() => {
      // Array of possible bot responses to make conversations feel natural
      // Each bot uses the same responses but with their own personality
      const responses = [
        `Hi! I'm ${selectedBot.name}. How can I help with your style today?`,
        `Great question! Let me share some ideas that might work for you.`,
        `I can suggest some combinations that match your preferences.`,
        `That's interesting! I have some thoughts on that.`
      ];
      
      // Create bot response message with random response from array
      // This adds variety to make each conversation feel unique
      const botMessage = {
        id: Date.now() + 1,              // Unique ID (slightly different from user message)
        text: responses[Math.floor(Math.random() * responses.length)], // Random response
        sender: 'bot',                   // Mark this as coming from the AI bot
        timestamp: new Date(),           // When the bot responded
        bot: selectedBot                 // Which bot sent this message (for styling)
      };
      setMessages(prev => [...prev, botMessage]);  // Add bot response to chat
      setIsTyping(false);                          // Hide typing indicator
    }, 1200); // 1.2 second delay to simulate thinking time
  };
  
  // Function to handle selecting a bot from the carousel
  // This starts a new conversation with the chosen AI stylist
  const handleBotSelect = (bot) => {
    setSelectedBot(bot);  // Set the chosen bot as the active assistant
    
    // Create welcome message from the selected bot
    // This immediately starts the conversation with a friendly greeting
    setMessages([{
      id: Date.now(),                    // Unique message ID
      text: `Hello! I'm ${bot.name}. I'm here to help you with your style. What would you like to explore?`,
      sender: 'bot',                     // This is a bot message
      timestamp: new Date(),             // Current time
      bot: bot                          // Reference to the bot for styling
    }]);
  };
